...
import Table from "./ChDMotorTable";
import '../../../style.css'
...
function ChDMotor() {
    ...
    const checkHuman = async (id) => {
        let res = await axios.get(`${config.url}/Motor/${id}`, {
            headers: {
    ...
    const fetchTecher = async () => {
        let res = await axios.get(`${config.url}/Motor`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
    ...
    return (
        ...
        <Breadcrumb.Item><Link to={"/adminpanel"}>Главная</Link></Breadcrumb.Item>
            ...
            <Breadcrumb.Item active>Моторные масла</Breadcrumb.Item>
          </Breadcrumb>
          <h1 className="h11">Моторные масла</h1>
          <div>
                <div className="bg-white dark:bg-gray-900">
                <div className="container w-full mx-auto py-12">
                        <div className="relative overflow-x-auto sm:rounded-lg">
                        <Table data={Motor} th={["Название", "№ Н/Д", "ИД НОМЕР"]} spTeacherFunc={checkHuman} spTeacher={spTeacher} />
                        </div>
                    </div>
                </div>
            </div>
