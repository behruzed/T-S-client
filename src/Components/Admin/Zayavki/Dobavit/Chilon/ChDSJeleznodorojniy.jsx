...
import Table from "./ChDSJeleznodorojniyTable";
import '../../../style.css'
import React from "react";
import Aside from "../../../Aside";
import Breadcrumb from 'react-bootstrap/Breadcrumb';
import {Link} from 'react-router-dom';
function ChDSJeleznodorojniy() {
    let [Jeleznodorojniy, setItem] = useState([])
    ...
    const checkHuman = async (id) => {
        let res = await axios.get(`${config.url}/Jeleznodorojniy/${id}`, {
            headers: {
    ...
    const fetchTecher = async () => {
        let res = await axios.get(`${config.url}/Jeleznodorojniy`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
        ...
        <Breadcrumb.Item><Link to={"/adminpanel"}>Главная</Link></Breadcrumb.Item>
          ...
          <div>
                ...
                <div className="container w-full mx-auto py-12">
                        ...
                        <Table data={Jeleznodorojniy} th={["Название", "№ Н/Д", "ИД НОМЕР"]} spTeacherFunc={checkHuman} spTeacher={spTeacher} />
                        </div>
                    </div>
                </div>
            </div>
...
export default ChDSJeleznodorojniy