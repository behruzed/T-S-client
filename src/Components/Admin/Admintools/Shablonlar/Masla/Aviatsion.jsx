...
import Table from "./AviatsionTable";
import '../../../style.css'
...
function Aviatsion() {
    ...
    const checkHuman = async (id) => {
        let res = await axios.get(`${config.url}/aviatsion/${id}`, {
            headers: {
    ...
    const fetchTecher = async () => {
        let res = await axios.get(`${config.url}/aviatsion`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
                ...
                <div>
                        <div className="bg-white dark:bg-gray-900">
                            <div className="container w-full mx-auto py-12">
                                ...
                                <Table data={Aviatsion} th={["Название", "Вид", "Категория", "№ Н/Д", "Ид номер"]} spTeacherFunc={checkHuman} spTeacher={spTeacher} />
                                </div>
                            </div>
                        </div>
                    </div>
