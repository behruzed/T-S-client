...
function Pr() {
  ...
  const admin = async () => {
    let data = await axios(`${config.url}/proizvodstvo`, {
      headers: {
  ...
  const fetchTecher = async () => {
    let res = await axios.get(`${config.url}/proizvodstvo`, {
      headers: {
        authorization: window.localStorage.getItem("token"),
      },
    });
    ...
    <>
      <div className="row mainrow">
        ...
        <Breadcrumb className="bread">
          ...
          <h1 className="h11">Бригадир смены: {localStorage.getItem("person")}</h1>
