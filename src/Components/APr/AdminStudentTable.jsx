...
export default ({ data, th, spStudentFunc, spStudent }) => {
    ...
    const profIcheck = () => { setProfI(!profI); };
    ...
    const editStudent = async () => {
        if (gcv(inp1) || gcv(inp2) || gcv(inp3) || gcv(inp4) || gcv(inp5) || gcv(inp6) || gcv(inp7)) {
            let data = {
                ...
                password: gcv(inp4).length === 0 ? undefined : gcv(inp4),
                phone: inp5.current.value.length === 0 ? undefined : inp5.current.value,
                ParentsPhoneNumber: {
                    // mother: inp6.current.value.length === 0 ? undefined : inp6.current.value,
                    // father: inp7.current.value.length === 0 ? undefined : inp7.current.value
                    mother: '911',
                    father: '911'
                }
            }
            let res = await axios.put(`${config.url}/students/${StudentId}`, data, {
                headers: {
            ...
            alert("Complete any input...")
        }
    };
    const delTeacher = async (id) => {
        let res = await axios.delete(`${config.url}/students/${id}`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
        if (res.data.title === "Student deleted") {
            alert(res.data.title)
        }
        window.location.reload()
    };
    const addStudent = async () => {
        if (gcv(inp1) && gcv(inp2) && gcv(inp3) && gcv(inp4) && gcv(inp5) && gcv(inp6) && gcv(inp7)) {
            ...
            let res = await axios.post(`${config.url}/students`, data, {
                headers: {
                    authorization: window.localStorage.getItem("token")
                }
            })
            if (res.data.title === "Student created") {
                alert(res.data.title)
            }
                    ...
                    {data.map((item, i) => {
                        return (
                            <tr key={item._id} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td className="px-6 py-4">
                                    {item.ParentsPhoneNumber.mother}
                                </td>
                                <td className="px-6 py-4">
                                    {item.ParentsPhoneNumber.father}
                                </td>
                                <td className="px-6 py-4 text-right w-32">
                                    ...
                                    <a onClick={prof1} onClickCapture={() => { spStudentFunc(item._id) }} data-modal-target="profile-modal" data-modal-toggle="profile-modal"
                    ...
                    <div className="w-full relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div className="w-full px-6 py-6 lg:px-8">
                            ...
                            <div className="forBtn">
                                <button onClick={editStudent} type="button" id="simple-search" className="buttun">Submit</button>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div className="px-8 py-6 lg:px-8">
                            <div className="w-full max-w-md p-3  border-none bg-gray-200 rounded-lg  dark:bg-gray-800 dark:border-gray-700 ">
                                <div className="student1 flex justify-end px-4 pt-4"></div>
                                <div className=" pb-10">
                                    ...
                                    <p className="mb-1 text-xl  text-gray-900 dark:text-white"><strong>First Name : </strong>{spStudent.firstName}</p>
                                    <p className="mb-1 text-xl  text-gray-900 dark:text-white"><strong>Last Name : </strong>{spStudent.lastName}</p>
                                    <p className="mb-1 text-xl  text-gray-900 dark:text-white"><strong>Email : </strong>{spStudent.email}</p>
                                    <p className="mb-1 text-xl  text-gray-900 dark:text-white"><strong>Phone : </strong>{spStudent.phone}</p>
