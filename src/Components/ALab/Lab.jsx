...
function Lab() {
  ...
  const admin = async () => {
    let data = await axios(`${config.url}/laboratoriya`, {
      headers: {
  ...
  const fetchTecher = async () => {
    let res = await axios.get(`${config.url}/laboratoriya`, {
      headers: {
        authorization: window.localStorage.getItem("token"),
      },
    });
    ...
    <>
      <div className="row mainrow">
        ...
        <Breadcrumb className="bread">
          ...
          <h1 className="h11">Лаборатория: {localStorage.getItem("person")}</h1>
