...
function LabNach() {
  ...
  const admin = async () => {
    let data = await axios(`${config.url}/nachalnik-lab`, {
      headers: {
  ...
  const fetchTecher = async () => {
    let res = await axios.get(`${config.url}/nachalnik-lab`, {
      headers: {
        authorization: window.localStorage.getItem("token"),
      },
    });
    ...
    <>
      <div className="row mainrow">
        ...
        <Breadcrumb className="bread">
          ...
          <h1 className="h11">Заведующий лабораторией: {localStorage.getItem("person")}</h1>
