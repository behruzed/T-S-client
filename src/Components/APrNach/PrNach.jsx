...
function PrNach() {
  ...
  const admin = async () => {
    let data = await axios(`${config.url}/nachalnik-pr`, {
      headers: {
  ...
  const fetchTecher = async () => {
    let res = await axios.get(`${config.url}/nachalnik-pr`, {
      headers: {
        authorization: window.localStorage.getItem("token"),
      },
    });
    ...
    <>
      <div className="row mainrow">
        ...
        <Breadcrumb className="bread">
          ...
          <h1 className="h11">Начальник производство: {localStorage.getItem("person")}</h1>
