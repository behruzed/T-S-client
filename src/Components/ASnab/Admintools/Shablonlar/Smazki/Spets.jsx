...
function Spets() {
    ...
    const checkHuman = async (id) => {
        let res = await axios.get(`${config.url}/spets/${id}`, {
            headers: {
    ...
    const fetchTecher = async () => {
        let res = await axios.get(`${config.url}/spets`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
        ...
        <>
            <div className="row mainrow">
                ...
                <Breadcrumb className="bread">
                    ...
                    <Breadcrumb.Item active>Спец. смазки</Breadcrumb.Item>
                </Breadcrumb>
                <h1 className="h11">Спец. смазки</h1>
                <div>
                        <div className="bg-white dark:bg-gray-900">
                        <div className="container w-full mx-auto py-12">
                                <div className="relative overflow-x-auto sm:rounded-lg">
                                    <Table data={Spets} th={["Название", "Категория", "Отделение", "Норма"]} spTeacherFunc={checkHuman} spTeacher={spTeacher} />
                                </div>
                            </div>
                        </div>
                    </div>
