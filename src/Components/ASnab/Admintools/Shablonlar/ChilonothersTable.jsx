...
// import Chilonothers from "./Chilonothers";
import "../../style.css";
...
export default ({ data, th, spTeacherFunc, spTeacher }) => {
  const getName1 = async (e, id) => {
    ...
    window.localStorage.setItem("nom", qwe.name);
    let data = {
      subject: "position",
      parent: window.localStorage.getItem("nom"),
    };
    let data1 = await axios.post(`${config.url}/others`, data, {
      headers: {
  ...
  const editTeacher = async () => {
    ...
    let res = await axios.put(`${config.url}/others/${teacherId}`, data, {
      headers: {
        authorization: window.localStorage.getItem("token"),
      },
    });
  ...
  const delTeacher = async (id) => {
    let res = await axios.delete(`${config.url}/others/${id}`, {
      headers: {
        authorization: window.localStorage.getItem("token"),
      },
    });
  ...
  const addTeacher = async () => {
    if (
      ...
      let res = await axios.post(`${config.url}/others`, data, {
        headers: {
          authorization: window.localStorage.getItem("token"),
        },
      });
