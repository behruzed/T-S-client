...
export default ({ data, th }) => {
    ...
    const editGroup = async () => {
        ...
        if (gcv(inp1) && gcv(inp2) && gcv(inp3)) {
            ...
            let res = await axios.put(`${config.url}/groups/${groupId}?idTeacher=${getId.data.id}`, groupEd, {
                headers: {
                    authorization: window.localStorage.getItem("token")
                }
            })
    ...
    const delStudent = async (id) => {
        ...
        let res = await axios.delete(`${config.url}/groups?idTeacher=${getId.data.id}&idGroup=${id}`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
    ...
    const addGroup = async () => {
        ...
        if (gcv(inp1) && gcv(inp2) && gcv(inp3)) {
            ...
            let res = await axios.post(`${config.url}/groups?idTeacher=${getId.data.id}`, addGr, {
                headers: {
                    authorization: window.localStorage.getItem("token")
                }
            })
                        ...
                        return (
                            <tr key={item._id} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td className="px-6 py-4">
                                    <a onClick={() => { getId(item._id, item.title) }} href="/teacheronegroup">{item.title}</a>
                                </td>
                                <td className="px-6 py-4">
                                    {item.day}
                                </td>
