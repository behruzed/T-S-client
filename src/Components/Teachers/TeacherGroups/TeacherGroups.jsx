...
import TeacherGroupTable from "./TeacherGroupTable";
...
function TeacherGroups() {
    ...
    const teacherGroups = async () => {
        ...
        let data = await axios(`${config.url}/groups?idTeacher=${getId.data.id}`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
                ...
                <div className="bg-white dark:bg-gray-900 h-screen pt-10">
                    ...
                    <div className="container w-10/12 mx-auto py-12">
                        <div className="relative overflow-x-auto shadow-xl sm:rounded-lg">
                            <TeacherGroupTable data={groups} th={["title", "day", "time", "students"]} />
                        </div>
                    </div>
                </div>
            </div>
