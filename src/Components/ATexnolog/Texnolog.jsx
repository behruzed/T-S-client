...
function Texnolog() {
  ...
  const admin = async () => {
    let data = await axios(`${config.url}/texnolog`, {
      headers: {
  ...
  const fetchTecher = async () => {
    let res = await axios.get(`${config.url}/texnolog`, {
      headers: {
        authorization: window.localStorage.getItem("token"),
      },
    });
    ...
    <>
      <div className="row mainrow">
        ...
        <Breadcrumb className="bread">
          ...
          <h1 className="h11">Технолог: {localStorage.getItem("person")}</h1>
