...
import Table from "./MoylarrTable";
import '../style.css'
import React from "react";

function Moylarr() {
    let [Moylarr, setItem] = useState([])
    ...
    const checkHuman = async (id) => {
        let res = await axios.get(`${config.url}/moylarr/${id}`, {
            headers: {
    ...
    const fetchTecher = async () => {
        let res = await axios.get(`${config.url}/moylarr`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
                        ...
                        <div className="bg-white dark:bg-gray-900">
                            <div className="container w-10/12 mx-auto py-12">
                                <div className="relative overflow-x-auto shadow-xl sm:rounded-lg">
                                    <Table data={Moylarr} th={["Название", "Категория", "Отделение", "Норма"]} spTeacherFunc={checkHuman} spTeacher={spTeacher} />
                                </div>
                            </div>
                        </div>
                    </div>
