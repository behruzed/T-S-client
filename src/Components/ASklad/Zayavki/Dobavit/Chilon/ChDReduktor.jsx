...
import Table from "./ChDReduktorTable";
import '../../../style.css'
import React from "react";
import Aside from "../../../Aside";
import Breadcrumb from 'react-bootstrap/Breadcrumb';

function ChDReduktor() {
    let [Reduktor, setItem] = useState([])
    ...
    const checkHuman = async (id) => {
        let res = await axios.get(`${config.url}/Reduktor/${id}`, {
            headers: {
    ...
    const fetchTecher = async () => {
        let res = await axios.get(`${config.url}/Reduktor`, {
            headers: {
                authorization: window.localStorage.getItem("token")
            }
        })
    ...
    return (
        ...
        <Breadcrumb className="bread">
            ...
            <Breadcrumb.Item active>Редукторные масла</Breadcrumb.Item>
          </Breadcrumb>
          <h1 className="h11">Редукторные масла</h1>
          <div>
                <div className="bg-white dark:bg-gray-900">
                <div className="container w-full mx-auto py-12">
                        <div className="relative overflow-x-auto sm:rounded-lg">
                        <Table data={Reduktor} th={["Название", "№ Н/Д", "ИД НОМЕР"]} spTeacherFunc={checkHuman} spTeacher={spTeacher} />
                        </div>
                    </div>
                </div>
            </div>
